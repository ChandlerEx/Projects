// CYD Yankees Scoreboard Prototype – Complete .ino with Simplified Touch & Serial Reset
    ...
    if (!deserializeJson(doc, http.getStream())) {
      ...
      if (dates.size() > 0 && dates[0]["games"].size() > 0) {
        ...
        if (gd != "") {
          ...
          time_t post = gameStartTime + 7 * 3600;
...
void setup() {
  ...
  if (firstBoot) {
    ...
    tft.println("* Connect to 'CYD-Scoreboard'");
    tft.println("* Go to 192.168.4.1");
    tft.println("* Follow prompts");
    wm.resetSettings();
    wm.setConfigPortalTimeout(180);
    if (!wm.autoConnect("CYD-Scoreboard")) ESP.restart();
    prefs.begin("cfg", false);
    ...
    if (WiFi.status() != WL_CONNECTED) {
      ...
      tft.println("* Follow prompts");
      wm.setConfigPortalTimeout(180);
      if (!wm.startConfigPortal("CYD-Scoreboard")) ESP.restart();
    }
  }
